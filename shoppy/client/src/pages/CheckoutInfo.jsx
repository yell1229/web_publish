...
export default function CheckoutInfo() {
    ...
    useEffect(() =>{
        if(isLoggedIn){
            getOrderList();// 전역 (에러 코트에 static이 나오면 전역을 가르킨다.
            //setMember(orderList[0]); // 로컬
        } 
    },[isLoggedIn]);
    ...
    <div className="section">
        ...
        <div className="info-grid">
            ...
            <div className="value">
                {
                    member.zipcode ?
                    <div className="value">{member.zipcode}{member.zonecode}</div>
                    :<div className="value">배송지를 입력해주세요.</div>
    ...
    <div className="section">
        ...
        {orderList && orderList.map((item,i) =>
            <div className="info-grid">
                ...
                <div className="value"><img src={item.image} alt="" style={{width:'100px'}} />{item.pname}{item.info}</div>
            </div>
        )}
        </div>
    </div>
